		...
		if (strcasecmp($_SERVER['REQUEST_METHOD'], 'post') == 0)
		{
			if(isset($_POST['env_key']) && isset($_POST['data']))
			{
				...
				try
				{
					...
					if ($objPmReq->objPmNotify->errorCode == 36)
					{
						$errorMessage = $objPmReq->objPmNotify->getCrc();
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mobilpay_order_status_canceled_id'));
					}
...
//							$this->model_checkout_order->update($order_id, $this->config->get('mobilpay_order_status_id'), '', TRUE);
						...
						case 'confirmed_pending': 
							...
							$errorMessage = $objPmReq->objPmNotify->getCrc();
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mobilpay_order_status_confirmed_pending_id'));
						...
						case 'paid_pending':
							...
							$errorMessage = $objPmReq->objPmNotify->getCrc();
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mobilpay_order_status_paid_pending_id'));
